...
mod tests {
    ...
    /// Exercises every named key through `key_type_string` (ported from the
    /// upstream `String()` behavior).
    #[test]
    fn test_key_type_string_table() {
        let cases: &[(u32, &str)] = &[
            (KEY_ENTER, "enter"),
            (KEY_TAB, "tab"),
            (KEY_BACKSPACE, "backspace"),
        ...
        for &(code, want) in cases {
            assert_eq!(key_type_string(code), Some(want), "code {code:#x}");
        }
        // Unknown codes return None.
        assert_eq!(key_type_string(99999), None);
        assert_eq!(key_type_string(0x1000000), None);
    }

    /// keystroke falls back to text/base_code for non-named keys.
    #[test]
    fn test_keystroke_fallbacks() {
        let k = Key {
            text: "hello".to_string(),
            code: KEY_EXTENDED,
            ..Key::default()
        };
        assert_eq!(k.keystroke(), "hello");
        let k = Key {
            code: KEY_SPACE,
            text: " ".to_string(),
            ..Key::default()
