...
fn helloworld_quits_on_any_key() {
    let mut pty = PtySession::spawn(&ex("helloworld"), &[]).expect("spawn");
...
fn altscreen_toggles_with_space() {
    let mut pty = PtySession::spawn(&ex("altscreen"), &[]).expect("spawn");
    pty.wait_for_text("inline mode", 5000).expect("inline");
    pty.press("space").expect("space");
    pty.wait_for_text("alternate screen mode", 5000).expect("alt screen");
    pty.press("space").expect("space");
    pty.wait_for_text("inline mode", 5000).expect("back to inline");
...
fn boxes_create_window_on_click() {
    let mut pty = PtySession::spawn(&ex("advanced_boxes"), &[]).expect("spawn");
    pty.wait_for_raw("?1002", 5000).expect("mouse mode");
    std::thread::sleep(std::time::Duration::from_millis(300));
    // Click in the root window: a new window is created.
    pty.send(&charming_testkit::keys::mouse_click(40, 12)).expect("click");
    pty.wait_for_raw("clicked root window", 5000).expect("window created");
    // Click again: now the new window is hit and focused.
    pty.send(&charming_testkit::keys::mouse_click(40, 12)).expect("click2");
    pty.wait_for_raw("clicked window", 5000).expect("window focused");
    // Right-click destroys it.
    pty.send(&charming_testkit::keys::mouse_right_click(40, 12)).expect("right click");
    pty.wait_for_raw("destroying", 5000).expect("window destroyed");
...
fn layout_keys_move_dialog() {
    let mut pty = PtySession::spawn(&ex("advanced_layout"), &[]).expect("spawn");
    ...
    pty.wait_until(5000, |s| s.contains("marmalade")).expect("still shown");
    std::thread::sleep(std::time::Duration::from_millis(300));
...
fn draw_example_supports_typing() {
    let mut pty = PtySession::spawn(&ex("draw"), &[]).expect("spawn");
    pty.wait_for_text("Draw Example", 5000).expect("help shown");
    // Press any key to dismiss the help.
    pty.press("space").expect("space");
    pty.wait_until(5000, |s| !s.contains("Welcome to Draw")).expect("help dismissed");
    pty.press("ctrl+c").expect("quit");
    pty.wait_for_exit(5000).expect("exit");
}
...
fn panic_example_quits() {
    let mut pty = PtySession::spawn(&ex("panic"), &[]).expect("spawn");
    pty.wait_for_text("Panicing after 5 seconds", 5000).expect("shown");
    pty.press("q").expect("q");
    pty.wait_for_exit(5000).expect("exit");
}
...
fn prependline_logs_events() {
    let mut pty = PtySession::spawn(&ex("prependline"), &[]).expect("spawn");
    pty.wait_for_text("Hello, World!", 5000).expect("title bar");
    pty.wait_for_raw("WindowSizeEvent", 5000).expect("event logged");
    pty.press("q").expect("q");
    pty.wait_for_exit(5000).expect("exit");
}
...
fn splits_renders_and_quits() {
    let mut pty = PtySession::spawn(&ex("advanced_splits"), &[]).expect("spawn");
    pty.wait_for_text("Horizontal Layout Example", 5000).expect("shown");
...
fn tv_renders_and_quits() {
    let mut pty = PtySession::spawn(&ex("advanced_tv"), &[]).expect("spawn");
...
fn draw_mouse_drawing() {
    ...
    pty.send(&charming_testkit::keys::mouse_drag(charming_testkit::keys::MOUSE_LEFT, 20, 5)).expect("drag");
    pty.send(&charming_testkit::keys::mouse_drag(charming_testkit::keys::MOUSE_LEFT, 30, 5)).expect("drag2");
    std::thread::sleep(std::time::Duration::from_millis(300));
