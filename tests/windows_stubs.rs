...
//! the fallback API compile-safe and verify that unavailable operations expose
//! stable, explicit platform errors.
...
#[test]
fn file_backed_poll_reader_uses_the_non_unix_fallback() {
    let mut reader = new_poll_reader(FdFile::stdin_file()).expect("fallback reader is available");
    assert!(!reader
        .poll(Some(Duration::from_millis(50)))
        .expect("fallback polling should not fail"));
}
